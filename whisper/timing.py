...
def dtw_cuda(x, BLOCK_SIZE=1024):
    ...
    cost = np.ones(N + M + 2, M + 2) * np.inf
    cost[0, 0] = 0
    cost = cost.to(x.device)
    trace = np.zeros_like(cost, dtype=np.int32)
...
def find_alignment(
    ...
    *,
    ...
    # heads * tokens * frames
    weights = np.stack([QKs[_l][_h] for _l, _h in model.alignment_heads.indices().T])
    weights = weights[:, :, : num_frames // 2]
    weights = (weights * qk_scale).softmax(dim=-1)
    std, mean = np.std_mean(weights, dim=-2, keepdim=True, unbiased=False)
